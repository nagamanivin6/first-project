...
//		echo $action->id; die;
        switch ($action->id) {
            ...
            case 'registration':
                $config = [
                    'steps' => ['profile',['address'=>['address'],'test'=>['test']], 'phoneNumber', 'user'],
                    'autoAdvance'   => false,
                    'defaultBranch' => false,
...
//			print_r($event);
			...
			if($event->step=="profile")
				{
					 $event->branches =[
									'test'  => WizardBehavior::BRANCH_DESELECT,
									'address' => WizardBehavior::BRANCH_SELECT,
...
//		$event->data = $this->render('registration\\'.$event->step, compact('event', 'model'));
    ...
    */
    ...
    {
        $modelName = 'frontend\\models\\wizard\\survey\\'.ucfirst($event->step);
        $model = new $modelName();
		echo "<br><br><br>".$event->step;
        $post = Yii::$app->request->post();
		...
		//		print_r($event); die;
            ...
            $view = ($model instanceof \frontend\models\wizard\survey\CatDog
                ? 'catDog'
                : $event->step
            );
    ...
    */
    ...
    {
        $modelName = 'frontend\\models\\wizard\\quiz\\Question';
